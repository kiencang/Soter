...
export class SimulatorVehicleTruckService {
  createTruck(scene: BABYLON.Scene) {
    ...
    const cabinBody = BABYLON.MeshBuilder.CreateBox('cabinBody', { width: 2.8, height: 2.5, depth: 2.8 }, scene);
    cabinBody.position.y = 1.95;
    cabinBody.position.z = 2.4;
    cabinBody.material = cabinMat;
    cabinBody.parent = truckNode;

    const cabinNose = BABYLON.MeshBuilder.CreateBox('cabinNose', { width: 2.8, height: 1.3, depth: 1.0 }, scene);
    cabinNose.position.set(0, 1.35, 4.3);
    cabinNose.material = cabinMat;
    cabinNose.parent = truckNode;

    // Windshield (Black reflective glass)
    const windshield = BABYLON.MeshBuilder.CreateBox('windshield', { width: 2.6, height: 1.0, depth: 0.1 }, scene);
    windshield.position.set(0, 2.5, 3.8);
    ...
    leftWindow.position.set(-1.41, 2.4, 2.4);
    ...
    rightWindow.position.set(1.41, 2.4, 2.4);
    ...
    lLight.position.set(-1.1, 1.0, 4.81);
    ...
    rLight.position.set(1.1, 1.0, 4.81);
    ...
    const trailer = BABYLON.MeshBuilder.CreateBox('trailer', { width: 3.0, height: 3.2, depth: 13.5 }, scene);
    trailer.position.set(0, 2.3, -5.8);
    ...
    const stripeLeft = BABYLON.MeshBuilder.CreateBox('stripeL', { width: 0.02, height: 0.3, depth: 13.5 }, scene);
    stripeLeft.position.set(-1.51, 1.2, -5.8);
    stripeLeft.material = yellowLightMat;
    stripeLeft.parent = trailerNode;

    const stripeRight = BABYLON.MeshBuilder.CreateBox('stripeR', { width: 0.02, height: 0.3, depth: 13.5 }, scene);
    stripeRight.position.set(1.51, 1.2, -5.8);
    stripeRight.material = yellowLightMat;
    stripeRight.parent = trailerNode;

    // 3. Mirror Bracket Wings
    // Left Mirror
    const mirrorBarL = BABYLON.MeshBuilder.CreateBox('mBarL', { width: 0.6, height: 0.08, depth: 0.08 }, scene);
    mirrorBarL.position.set(-1.6, 2.5, 3.1);
    mirrorBarL.parent = truckNode;

    const mirrorPlateL = BABYLON.MeshBuilder.CreateBox('mPlateL', { width: 0.08, height: 0.8, depth: 0.22 }, scene);
    mirrorPlateL.position.set(-1.9, 2.5, 3.1);
    mirrorPlateL.material = tireMat;
    mirrorPlateL.parent = truckNode;

    // Right Mirror
    const mirrorBarR = BABYLON.MeshBuilder.CreateBox('mBarR', { width: 0.6, height: 0.08, depth: 0.08 }, scene);
    mirrorBarR.position.set(1.6, 2.5, 3.1);
    mirrorBarR.parent = truckNode;

    const mirrorPlateR = BABYLON.MeshBuilder.CreateBox('mPlateR', { width: 0.08, height: 0.8, depth: 0.22 }, scene);
    mirrorPlateR.position.set(1.9, 2.5, 3.1);
    ...
    frontMirrorBar.position.set(0.6, 3.0, 4.4);
    ...
    frontMirrorPlate.position.set(0.6, 3.3, 4.7);
    frontMirrorPlate.material = tireMat;
    frontMirrorPlate.parent = truckNode;

    // 4. Heavy Truck Wheels (Black tires)
    const tirePositions = [
      { x: -1.4, y: 0.6, z: 4.0 },  { x: 1.4, y: 0.6, z: 4.0 },
      { x: -1.4, y: 0.6, z: 1.5 },  { x: 1.4, y: 0.6, z: 1.5 },
      { x: -1.4, y: 0.6, z: -4.5 }, { x: 1.4, y: 0.6, z: -4.5 },
      { x: -1.4, y: 0.6, z: -6.0 }, { x: 1.4, y: 0.6, z: -6.0 },
      { x: -1.4, y: 0.6, z: -10.5 }, { x: 1.4, y: 0.6, z: -10.5 },
      { x: -1.4, y: 0.6, z: -12.0 }, { x: 1.4, y: 0.6, z: -12.0 }
    ];
    ...
    tailL.position.set(-1.1, 0.8, -12.56);
    ...
    tailR.position.set(1.1, 0.8, -12.56);
